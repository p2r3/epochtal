 ...
 * @property {string[]} [targetnames] Normalized targetname must match one in this list
 * @property {number[][]} [locations] Origin coordinates of the entity must appear in this list; omit to skip origin check
...
/**
 * Standard PeTI or BEEmod entities that end the map.
 * When checking for methods to end a PeTI or BEEmod map, these entities are excluded as
 * they do not bypass the exit door.
 */
...
/**
 * @param {object} entity
 * @param {number[][]|undefined} locations
 * @returns {boolean}
 */
function entityOriginMatchesLocations (entity, locations) {

  if (locations == null) return true;
  if (!Array.isArray(entity.origin)) return false;
  return locations.some(loc => loc.every((v, i) => v === entity.origin[i]));
 ...
 * @returns {string[]}
 */
function outputTargetsFromEntity (entity) {

  return [...iterableOfConnectionsFromEntityOutputs(entity, Object.keys(entity.outputs ?? {}))].map(c => c[0]);
 ...
 * @returns {string[]}
 */
function normalizedOutputTargetsFromEntity (entity) {

  return outputTargetsFromEntity(entity).map(t => removeAllNumbers(t));

}

/**
 * @param {object} entity
 * @param {string} tgName Normalized lowercase targetname
 * @returns {boolean}
 */
function isStandardMapEndEntity (entity, tgName) {

  for (const rule of STANDARD_PETI_OR_BEEMOD_MAP_END) {
    if (entity.classname !== rule.classname) continue;
    if (rule.targetnames != null && !rule.targetnames.includes(tgName)) continue;
    if (!entityOriginMatchesLocations(entity, rule.locations)) continue;

    const normalizedTargets = normalizedOutputTargetsFromEntity(entity);

    if (rule.outputEvery != null) {
      if (!rule.outputEvery.every(t => normalizedTargets.includes(t))) continue;
    }
    else if (rule.outputSome != null) {
      if (!rule.outputSome.some(t => normalizedTargets.includes(t))) continue;
    }
...
function canStartInputOutputChain (entity, entitiesThatArePermanentlyDisabled) {
  ...
  if (entity.classname === "logic_relay") {
    ...
    if (removeAllNumbers(entity.targetname) === "pro_lfs-out") return true;
    // A logic relay can start an input-output chain only if it has an 'OnSpawn' output.
    if (Object.keys(entity.outputs ?? {}).some(o => o === "onspawn")) return true;
    return false;
  }
...
async function isMapSolvable (data, context = epochtal) {
  ...
  for (const entity of entities) {
    ...
    if (entitiesThatArePermanentlyDisabled.has(entity)) targetedEntityInputs = [];
      ...
      // - unless this is a Hammer map, where these start-of-map connections may both alter these
      // values and use these values, so logic relays' parameters may be used at a non-starting value
  ...
  for (const entity of entities) {
    ...
    for (const entityInput of targetedEntityInputs) {
      ...
      if (
        ...
        if (!isHammerMap && isStandardMapEndEntity(entity, tgName)) continue;
  ...
  for (let regionNumber = 0; regionNumber < numRegions; regionNumber++) {
    ...
    if (!hasCubeDropperThatCanDrop && !hasSphereDropperThatCanDrop) {
      ...
      if (entranceRegionNumbers.has(regionNumber)) {
        ...
        if (!isBEEmodMap && !entities.some(entity => entity.targetname === "@door_wants_close"))
          numPossibleSticks = 0;
